...
const ranges = [
  { label: 'Prochain 7 jours', days: 7 },
  { label: 'Prochain 14 jours', days: 14 },
  { label: 'Prochain 30 jours', days: 30 },
  { label: 'Prochain 3 mois', months: 3 },
  { label: 'Prochain 6 mois', months: 6 },
  { label: 'Prochaine année', years: 1 }
]
...
const isRangeSelected = (range: { days?: number, months?: number, years?: number }) => {
  ...
  if (range.days) {
    startDate = startDate.add({ days: range.days })
  } else if (range.months) {
    startDate = startDate.add({ months: range.months })
  } else if (range.years) {
    startDate = startDate.add({ years: range.years })
  }
